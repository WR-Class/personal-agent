...
给操作者的动作：若该聚合站不应该对本地无鉴权开放，请在服务侧开启校验；若该密钥曾被用于任何可外泄的位置（包括本会话的命令行与终端输出），请轮换它。（2026-09-25 操作者已决定：构建与测试阶段继续在本机使用该端点与密钥做真实调用；真实产品不内置任何凭据——配置只存在于本机 `.personal-agent/`，已被 git 忽略。）
