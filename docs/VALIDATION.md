...
- **两次未解释的偶发失败（诚实记录）**：在"推理可见性"批的全量测试中，出现过**一次** `reasoning traces…reads the trace and its token share` 在约 3ms 内失败；在"版本控制与 CI"批的一次全量运行中，出现过**一次 `fail 3`**（可惜那次输出被后续命令覆盖，**未留存**，因此那次失败的具体用例与报错文本不可知——这是我的操作失误，不是测试问题：失败输出必须先另存再继续）。此后连续 **17 次**全量运行（含并行加压、含干净克隆内的一次）全部 200/0/1，**两者均未能复现**。已做的排查：(a) 测试辅助函数原先把 `server.listen` 的错误交给未处理的 `'error'` 事件（会被归因到当时正在运行的任意测试），现已改为显式拒绝并报出原因，并已用"占用端口后再监听"的探针**证实**该守卫会报出 `EADDRINUSE`；(b) 已通读 `test/fixtures.ts` 排除共享状态假设——`mkdtemp` 为每个 fixture 生成唯一目录，跨进程并发不冲突，**该假设被排除**。结论：**根因未能确证**；能保证的只有"再次出现时会指名原因"，以及下一次失败输出会被留存。
