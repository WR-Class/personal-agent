...
- **一次未解释的偶发失败（诚实记录）**：在"推理可见性"批的全量测试中，出现过**一次** `reasoning traces…reads the trace and its token share` 在约 3ms 内失败，随后连续 10 次全量运行（含并行加压）均通过，**未能复现**。失败快到不像断言不符，更像在建立测试服务器前就抛出。测试辅助函数原先把 `server.listen` 的错误交给未处理的 `'error'` 事件（会被归因到当时正在运行的任意测试），现已改为显式拒绝并报出原因，并已用一个"占用端口后再监听"的探针**证实**该守卫会报出 `EADDRINUSE`。因此：**该偶发失败的根因未能确证**，只能保证再次出现时会指名原因。
