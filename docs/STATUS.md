...
4. **进行中（用户已选）**：①推理可见性——**已完成**；②SSE 流式——**已完成**（传输层；**非**增量显示，见 [CLI_STATUS](CLI_STATUS.md)）；③Git 版本控制与 CI——**已完成**（仓库在 `personal-agent/` 内，**不含**工作区根目录的 `_research`/`restore` 等外部参考树；CI 从未在真实运行器上执行过）。Node 22 下限已用官方 **22.23.3 便携包**在干净克隆上实测（`npm ci` → 类型检查 → 207/206/0/1），解压在 `.test-artifacts/toolchain/`，**未改全局 Node**；测的是这一版，不是整个 22.x 区间，也不是 22.6 下限本身。
