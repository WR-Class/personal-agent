...
| M2 权限与受控文件修改 | **当前阶段，未收口** | 已落地：`edit_file`、`patch_file`、`create_file`、`delete_file`、`rename_file`、`batch_files`。每次批准有 diff 或清单，2 分钟过期 |
...
1. **收口 M2**：六个文件操作共用一个批准函数。grant 必须同时匹配工具名、参数 JSON 和到期时间。仍缺路径竞态、保护目录专项测试、拒绝审计事件。
2. **TaskSpec**：发送前记录原输入、目标、意图、模式。缺目标不编造模式。强制拒绝先关闭。
