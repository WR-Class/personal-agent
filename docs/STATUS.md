...
**当前为可持续对话、带安全只读文件工具与会话保护的独立单 Agent CLI，处于 M1。** 2026-09-25 用户已确认 M1 验收。这不等于 M2/M3 已开放：写文件、进程、插件、多 Agent 仍未开始。不是 API 代理，不依赖 DSH 运行时。
...
| M2 权限与受控文件修改 | 第一批进行中 | 只替换一个已有 UTF-8 文件：批准前显示最多 40 行差异，回答“是”后 2 分钟内用临时文件加替换写入。不新建、不删除、不重命名、不批量改 |
...
1. **兼容与工程化**：Git 与 GitHub Actions 已落地（Windows + Node 22.x/24.x 首跑成功，见 [run #1](https://github.com/WR-Class/personal-agent/actions/runs/36145175526)）。发行构建、旧会话 fixtures、非 Windows 平台实测仍未做。
2. **真实 Provider 与真实 TTY 联调**：本机回环端点、真实工具闭环、流式工具往返已测。**仍未执行**：外部托管 Provider、实体 TTY 自动化。操作者已在真实终端手工验证过交互与取消。细节见 [联调 Runbook](LIVE_INTEGRATION.md)。
3. **M1 总验收**：**用户已确认（2026-09-25）。** 确认的是受信只读范围。未覆盖项仍列在本表与 [VALIDATION](VALIDATION.md)，不因此开放 M2/M3。
4. **加固三项均已完成**：①推理可见性；②SSE 传输（**非**增量显示）；③Git 与 CI。Node 22.23.3 便携包已在干净克隆实测，**未改全局 Node**。
5. **仍待办**：取消中的轮次不打印用量/耗时；默认 Agent home 在项目树内，明文密钥落在 `.personal-agent/`（已被 git 忽略，但发行时仍须显式排除）。
