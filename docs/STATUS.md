...
最新批次：**版本控制与 CI**（仓库只覆盖 `personal-agent/`，50 个跟踪文件；`.gitattributes` 固定 LF 使完整性哈希可复现；CI 为 Windows + Node 22.x/24.x 的 build+test）。此前批次：SSE 流式传输、推理可见性、`--preflight` 联调准备检查、持久性语义、PT04 路径收敛、PT06 审计重复检查、按模型窗口与剩余量显示、可注入 tokenizer 精确预判、摘要保真压缩、本机 socket 与真实端点 Provider 联调。原文档统一基线：2026-09-24。此文件负责阶段/剩余任务；其他主文档负责各自专题，批次记录保留历史证据。后续每批实现必须先更新本表。
...
4. **进行中（用户已选）**：①推理可见性——**已完成**；②SSE 流式——**已完成**（传输层；**非**增量显示，见 [CLI_STATUS](CLI_STATUS.md)）；③Git 版本控制与 CI——**已完成**（仓库在 `personal-agent/` 内，**不含**工作区根目录的 `_research`/`restore` 等外部参考树；两个提交；CI 从未在真实运行器上执行过，且 Node 22.x 未在本机测过）。
